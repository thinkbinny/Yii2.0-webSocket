...
namespace yii\websocket\swoole;
use yii\base\InvalidConfigException;
use yii\websocket\cli\WebSocket as CliWebSocket;
...
class WebSocket extends CliWebSocket
{
    ...
    public function init()
    {
        ...
        if (!isset($this->host)) {
            throw new InvalidConfigException('Host parameter does not exist.');
        }

        if (!isset($this->port)) {
            throw new InvalidConfigException('Port parameter does not exist.');
        }
    ...
    public function send($data, $type = 'text', $masked = true)
    {
        ...
        // 将WebSocket消息打包并发送
        return $this->_socket->send(\swoole_websocket_server::pack(json_encode($data), $_type, $masked));
